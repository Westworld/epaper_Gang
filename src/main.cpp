...
void showImage() {
    ...
    //sprintf(logString,"Strom?Job=GangBitmap2&temp=%f", temp);
    //showBitmapDirectFrom_HTTP("192.168.0.34", "/4DAction/", logString, 0,0, false);

    sprintf(logString,"Strom?Job=GangBitmap&temp=%f", temp);
    showBitmapBufferFrom_HTTP("192.168.0.34", "/4DAction/", logString, 0,0, false);

    //sprintf(logString,"Strom?Job=Gangxmp&temp=%f", temp);
    //showXBMFrom_HTTP("192.168.0.34", "/4DAction/", logString, 0,0, false);
...
void showXBMFrom_HTTP(const char* host, const char* path, const char* filename, int16_t x, int16_t y, bool with_color)
{
  ...
  if (!connection_ok) return;
  // Parse BMP header
  uint16_t fileSize = read16(client);
  Serial.print("fileSize "); Serial.println(fileSize);

  if (fileSize >= 16000) return;

      uint16_t bytes_read = read(client, bmpbuffer, fileSize);
      Serial.print("bytes read "); Serial.println(bytes_read);
...
void showBitmapDirectFrom_HTTP(const char* host, const char* path, const char* filename, int16_t x, int16_t y, bool with_color)
{
  display.setRotation(0);
  WiFiClient client;
  bool connection_ok = false;
...
//  *****  begin
        bool whitish;
        uint8_t bitmask = 0xFF;
        uint8_t bitshift = 1; // 8- depth;
        display.clearScreen();

        for (uint16_t row = 0; row < width; row++) // for each line
        {
          yield(); // to avoid WDT
          ...
          for (uint16_t col = 0; col < width; col++) // for each pixel
          {
            ...
            if ((7 == col % 8) || (col == width - 1)) // write that last byte! (for w%8!=0 boarder)
            {
              output_row_mono_buffer[out_idx++] = out_byte;
              Serial.print(out_byte, HEX);
              out_byte = 0xFF; // white (for w%8!=0 boarder)
          ...
          int16_t yrow = y + (flip ? width - row - 1 : row);
          display.writeImage(output_row_mono_buffer, x, yrow, width, 1);
